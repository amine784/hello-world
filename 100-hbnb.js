...
$(document).ready(function () {
 ...
 const myCities = {};
 let myListCities = [];
 const checkbox = $('.locations input[type="checkbox"]');
 checkbox.prop('checked', false);
 checkbox.change(function () {
   myListCities = [];
   ...
   if (this.checked) {
     myCities[cityId] = cityName;
   } else {
     delete (myCities[cityId]);
   }
   for (const key in myCities) {
     myList_cities.push(myCities[key]);
   }
   const outputCities = myListCities.join(', ');
   $('div.locations > h4').text(outputCities);
});
 /** search state
     filterr
 **/
 ...
 checkbox.change(function () {
   ...
   if (this.checked) {
     myState[stateId] = stateName;
   } else {
     delete (myState[stateId]);
   }
   for (const key in myState) {
     myListState.push(myState[key]);
   }
   const outputState = myListState.join(', ');
   $('div.locations > h4').text(outputState);
   });
  /**
     * Task 3:
